...
const logger = winston.createLogger({
  ...
  transports: [
    new winston.transports.File({ filename: '/mobile.dev/api/side_cross_logs2.txt' }),
  ],
});
...
const authenticate = (req, res, next) => {
  ...
  try {
    ...
    const [ipResults] = await db.query('SELECT * FROM faucetCross WHERE ipAddress = ? AND status = "blocked"', [ipAddress]);
    ...
    const [addressResults] = await db.query('SELECT * FROM faucetCross WHERE address = ? AND status = "blocked"', [walletAddress]);
    ...
    const externalApiResponse = await axios.post('http://167.235.**.236:8002', { address: walletAddress });
    console.log("Response received from external API.");

    await db.query('INSERT INTO faucetCross (address, ipAddress, time, status) VALUES (?, ?, ?, ?)', [walletAddress, ipAddress, currentTime, 'blocked']);
...
app.get('/cross_faucet_logs', (req, res) => {
  logger.info('Log file requested');
  res.sendFile(path.join(__dirname, '/mobile.dev/api/cross_faucet_logs.txt'));
});
