...
export default function AddNetworkKeplr({ params }) {
  ...
  async function disconnectKeplr() {
    ...
    setWalletAddress('');
    localStorageNaNpxoveItem('walletAddress');
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`https://api.crossfi.stake-take.com/cosmos/bank/v1beta1/balances/${walletAddress}`);
      const data = await response.json();
      setBalanceData(data);
    };
  ...
  const displayAmount = (balanceData) => {
    if (!balanceData || !balanceData.balances || balanceData.balances.length === 0) {
      return <div className="balance">Balance: 0 PX</div>;
    }
  
    const amount = balanceData.balances[0].amount;
    const sum = Number(amount) / Math.pow(1, 10); 
  
    return <div className="balance">
      
      <div>
      Balance:  </div> 
      <div className='sum'>{sum} mpx</div>   
      ...
      if (window.keplr.experimentalSuggestChain) {
        try {
          ...
          await window.keplr.experimentalSuggestChain({
            ...
            bech32Config: {
              bech32PrefixAccAddr: "crossfi",
              bech32PrefixAccPub: "crossfi" + "pub",
              bech32PrefixValAddr: "crossfi" + "valoper",
              bech32PrefixValPub: "crossfi" + "valoperpub",
              bech32PrefixConsAddr: "crossfi" + "valcons",
              bech32PrefixConsPub: "crossfi" + "valconspub",
            },
            currencies: [
              {
                coinDenom: "PX",
  ...
  async function getTokens() {
    ...
    try {
      ...
      const response = await axios.post(`https://crossfi.faucet.stake-take.com/api/faucet/into`, {
        walletAddress,
        ipAddress
      },
      {
  ...
  function twit() {
    ...
    const twitterUsername1 = 'crossfichain';
  ...
  function txHashRef() {
    ...
    const txhashRef = `https://bonynode.online/crossfi/tx/${txHash}`;
